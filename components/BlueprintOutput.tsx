...
function CodeBlock({ className, children }: { className?: string; children: React.ReactNode }) {
  ...
  return (
    <div className="relative group">
      <button
        onClick={handleCopy}
        className="absolute top-2 right-2 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5"
      >
        {copied ? (
          <>
      ...
      <code className={`${className} block bg-black text-white p-4 rounded-lg overflow-x-auto text-sm font-mono`}>
        {children}
      </code>
    </div>
  );
...
export default function BlueprintOutput({ blueprint, projectIdea, blueprintId: propBlueprintId }: BlueprintOutputProps) {
  const [copied, setCopied] = useState(false);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap justify-end gap-3">
        <button
          onClick={handleSave}
          className="px-4 md:px-6 py-2 bg-black hover:bg-gray-800 text-white font-medium rounded-lg transition-colors duration-200 flex items-center gap-2 border border-gray-700"
        >
          {saved ? (
            <>
        ...
        <button
          onClick={handleCopy}
          className="px-4 md:px-6 py-2 bg-gray-700 hover:bg-gray-600 text-white font-medium rounded-lg transition-colors duration-200 flex items-center gap-2 border border-gray-600"
        >
          {copied ? (
            <>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          className="px-4 md:px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white font-medium rounded-lg transition-colors duration-200 flex items-center gap-2 border border-purple-500"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <ShareModal
        ...
        blueprintId={blueprintId || Date.now()}
        vibe={projectIdea}
      />
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2 bg-black hover:bg-gray-800 text-white font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {exporting ? (
                    <>
      ...
      <BuildFullAppButton
        ...
        blueprintId={blueprintId ? String(blueprintId) : propBlueprintId}
      />
    </div>
  );
}
